import { InstallBase, RootLauncherConfig } from "../../../types";
...
export class SharedInstaller {
  ...
  public async execute(arg: InstallBase, config: RootLauncherConfig) {
  ...
  private getPrefix(prefix: string, config: RootLauncherConfig) {
    if (config.dryRun) {
      return "";
    }
  ...
  private skipInstall(config: RootLauncherConfig) {
    if (config.packageManager === "npm") {
  ...
  private npmCommand(arg: InstallBase, config: RootLauncherConfig) {
  ...
  private dryRunOrSkipIstall(config: RootLauncherConfig) {
    if (config.dryRun) {
      return this.dryRun();
    }
