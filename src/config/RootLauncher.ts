...
import {
  ICLIConfig,
  RootLauncherConfig,
  RootLauncherTemplate,
  RootLauncherVariant,
} from "../types";
import { flags, inputs } from "../utils/cli";

class RootLauncher implements RootLauncherConfig {
  // Initial Values
  #auth: boolean = false;
  #base: boolean = false;
  #views: boolean = false;
  ...
  get variant(): RootLauncherVariant {
    if (this.#auth) {
  ...
  get template(): RootLauncherTemplate {
    if (this.fs) {
      return "fullstack";
    }
...
const RootLauncherConfig = new RootLauncher(flags, inputs);

export default RootLauncherConfig;
